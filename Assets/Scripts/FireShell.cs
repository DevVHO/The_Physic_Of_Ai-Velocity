...
public class FireShell : MonoBehaviour
{
    ...
    void CreateBullet()
    {
        GameObject shell = Instantiate(bullet, turretBase.transform.position, turretBase.transform.rotation);
    ...
    float? CalculateAngle(bool low)
    {
        ...
        float underThesSqrRoot = (sSqr * sSqr) - gravity * (gravity * x * x + 2 * y * sSqr);

        if (underThesSqrRoot >= 0f)
        {
            float root = Mathf.Sqrt(underThesSqrRoot);
